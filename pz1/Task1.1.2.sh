...
declare -r MY_CYTHON_PROJECT='task2'
...
void find_longest_word(char * filename) {
    FILE *file;
    char word[MAX_LEN], longest_word[MAX_LEN];
    ...
    printf("The first longest word is: %s\n", longest_word);
...
cdef extern from "examples.h":
    void find_longest_word(const char *name)

def py_find_longest_word(name: bytes) -> None:
    find_longest_word(name)
...
pyexamples.py_find_longest_word(b"../input.txt")
